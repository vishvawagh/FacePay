...
if (currentUser) {
  ...
  swal("Login First!", "To start face verification, please log in!\n\nPressing 'OK' will redirect you to log in.", "warning", { timer: 4000 }).then(function () {
...
//-----------------------------------------face-verification--------------------------------------------//
message.innerText = "Starting Camera.../कॅमेरा सुरू होत आहे...";
...
Promise.all([
  ...
  faceapi.nets.faceExpressionNet.loadFromUri(modelsSrc), // Load expression model for smile detection
]).then(startVideo);
...
function loadAndLabelImagesFromDB() {
  ...
  return Promise.all(
    labels.map(async (label) => {
      const descriptions = [];
      message.innerText = `Processing Data.../डेटा प्रक्रिया करत आहे...`;
...
async function matchFace() {
  ...
  message.innerText = "Data Processed! and Camera Started! / डेटा प्रक्रिया झाली! आणि कॅमेरा सुरू झाला!";
  setTimeout(() => {
    message.innerText = "Verifying.../ ओळख पटवत आहे...";
...
async function startFaceRecognition() {
  ...
  const recognizing = async () => {
    ...
    if (!resizedDetections[0]) {
      swal("Face not detected!", "Please take the shot near a better light source.\nOr, Try removing the spectacles/glasses or mask./ चेहरा सापडला नाही! कृपया चांगल्या प्रकाश स्रोताजवळ फोटो घ्या. किंवा, चष्मा/मास्क काढून पहा.", "warning");
    return;
      return;
    }
    ...
    if (faceLabel === "known" && faceScore <= 0.45 && smileScore > 0.3) { // Added smileScore check
      faceVerified = true;
      swal(`${currentUser.fullname}, you are now verified and smiling !! / / आपली ओळख पटली आहे.`, "", "success");
      setTimeout(() => {
        window.location.replace("/src/payment.html");
      }, 1000);
    } else if (faceLabel === "known" && (faceScore > 0.55 || smileScore <= 0.2)) {
      faceVerified = false;
      message.innerText = `Try again! We want to be more sure that it's ${currentUser.fullname} and you are smiling. / परत हसावे !`;
      setTimeout(() => {
        startFaceRecognition(); // Retry face recognition
      }, 2000); // Retry after 2 seconds
    } else {
      faceVerified = false;
      swal("Verification Failed!", "Face was not matched with the profile or you are not smiling. / चेहरा प्रोफाइलशी जुळला नाही, तुम्ही हसला नाहीत!", "error").then(() => {
