...
async function recognize() {
  ...
  if (faceLabel === "known" && faceScore <= 0.45) { // Increased threshold to 0.75 for easier matching
    faceVerified = true;
    swal(`${currentUser.fullname}, you are now verified! / आपली ओळख पटली आहे.  `, "", "success");
    setTimeout(() => {
      window.location.replace("/src/home.html");
    }, 1000); // Redirect after 1 second
  } else if (faceLabel === "known" && faceScore > 0.55) {
    faceVerified = false;
    message.innerText = `Try again! We want to be more sure that it's ${currentUser.fullname}./ पुन्हा प्रयत्न करा! आम्हाला अधिक खात्री करायची आहे.`;
    setTimeout(() => {
    ...
    faceVerified = false;
    swal("Verification Failed!", "Face was not matched with the profile / चेहरा प्रोफाइलशी जुळला नाही.", "error").then(() => {
